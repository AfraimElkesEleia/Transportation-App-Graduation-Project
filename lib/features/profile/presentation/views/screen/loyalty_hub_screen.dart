...
class _LoyaltyHubScreenState extends State<LoyaltyHubScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _challengesScroll.addListener(() {
      if (_challengesScroll.position.pixels >= _challengesScroll.position.maxScrollExtent - 200) {
        context.read<LoyaltyHubCubit>().loadChallenges();
      }
    });

    _pointsScroll.addListener(() {
      if (_pointsScroll.position.pixels >= _pointsScroll.position.maxScrollExtent - 200) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Loyalty Hub', style: TextStyle(color: Colors.white, fontSize: 18)),
  ...
  Widget _buildChallengesTab(LoyaltyHubLoaded state) {
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              FilterChip(
                ...
                onSelected: (val) => context.read<LoyaltyHubCubit>().toggleChallengeFilter(false),
                selectedColor: ColorsManager.accentCyan.withValues(alpha: 0.2),
                checkmarkColor: ColorsManager.accentCyan,
                labelStyle: TextStyle(
                  color: !state.showCompleted ? ColorsManager.accentCyan : Colors.white70,
                ),
                backgroundColor: ColorsManager.surfaceMid,
              ),
              ...
              FilterChip(
                ...
                selected: state.showCompleted,
                onSelected: (val) => context.read<LoyaltyHubCubit>().toggleChallengeFilter(true),
                selectedColor: ColorsManager.successGreen.withValues(alpha: 0.2),
                checkmarkColor: ColorsManager.successGreen,
                labelStyle: TextStyle(
                  color: state.showCompleted ? ColorsManager.successGreen : Colors.white70,
                ),
                backgroundColor: ColorsManager.surfaceMid,
              ),
            ],
        ...
        Expanded(
          child: state.challenges.isEmpty
              ? const Center(child: Text('No challenges found.', style: TextStyle(color: Colors.white70)))
              : ListView.builder(
                  controller: _challengesScroll,
                  padding: const EdgeInsets.all(16),
                  itemCount: state.challenges.length + (state.isLoadingMoreChallenges ? 1 : 0),
  ...
  Widget _buildPointsTab(LoyaltyHubLoaded state) {
    if (state.points.isEmpty) {
      return const Center(child: Text('No point history.', style: TextStyle(color: Colors.white70)));
    }
